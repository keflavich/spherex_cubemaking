...
def main() -> None:
    ...
    # All-spectra overlay (only ones with row info, color by Teff)
    STACK_DIR.mkdir(exist_ok=True)
    fig, ax = plt.subplots(figsize=(11, 6))
    teffs = np.array([row["Teff"] for _, row, _ in summaries if row is not None])
    ...
    ax.set_xlabel("λ (μm)")
    ax.set_ylabel("Flux (μJy)")
    ax.set_title(f"All {len(summaries)} SPHEREx spectra so far  (color = Teff)")
    ax.grid(True, alpha=0.3)
    ...
    print(f"{'APOGEE_ID':<22s} {'Teff':>6s} {'logg':>5s} {'[Fe/H]':>6s} "
          f"{'K':>5s} {'age':>5s} {'n_meas':>7s} {'clean':>6s} {'overflow':>9s}")
    for apid, row, s in summaries:
        if row is None:
        ...
        print(f"  {apid:<20s} {row['Teff']:>6.0f} {row['logg']:>5.2f} "
              ...
              f"{s['n_meas']:>7d} {s['n_clean']:>6d} {s['n_overflow']:>9d}")
